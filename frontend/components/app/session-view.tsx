...
function PlayerBadge({ participant }: { participant?: LocalParticipant }) {
  ...
  useEffect(() => {
    ...
    // Initial set
    updateName();
    // Listen for proper LiveKit v1.x participant events
    participant.on('participantNameChanged', updateName);
    participant.on('participantMetadataChanged', updateName);

    return () => {
      participant.off?.('participantNameChanged', updateName);
      participant.off?.('metadataChanged' as any, updateName);
    };
  }, [participant]);
  ...
  return (
    ...
    >
      ...
      <MotionBottom
        ...
        transition={{
          ...BOTTOM_VIEW_MOTION_PROPS.transition,
          // framer-motion expects `ease` to be an array or function, not string
          ease: [0.42, 0, 0.58, 1], // use a cubic-bezier as default "ease"
        }}
      >
